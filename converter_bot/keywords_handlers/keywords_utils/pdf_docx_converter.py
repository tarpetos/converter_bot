...
class DocxToPdf(FileConverter):
    ...
    def convert(self, conversion_file_path: str, paths: List[str], **kwargs) -> None:
        ...
        task = loop.create_task(self._parse_page(conversion_file_path, paths))
        task.add_done_callback(self._task_handler)

    @staticmethod
    def _task_handler(task: asyncio.Task) -> None:
        try:
            task.result()
        except (TimeoutException, TelegramNetworkError) as e:
            print(repr(e))
    ...
    async def _parse_page(self, conversion_file_path: str, paths: List[str]) -> None:
        ...
        file_input = WebDriverWait(driver, self.LOAD_TIMEOUT).until(
            expected_conditions.presence_of_element_located((By.XPATH, upload_file_button_xpath))
        )
        file_input.send_keys(conversion_file_path)

        download_file_button_xpath = "//div//span[contains(text(), 'Download')]"
        file_output = WebDriverWait(driver, self.LOAD_TIMEOUT).until(
            expected_conditions.element_to_be_clickable((By.XPATH, download_file_button_xpath))
        )
        file_output.click()
        await asyncio.sleep(self.LOAD_TIMEOUT)
        self.rename_pdf(conversion_file_path)

    @staticmethod
    def rename_pdf(docx_file_path: str) -> None:
        pdf_file_path = os.path.splitext(docx_file_path)[0] + f".{PDF}"
        if os.path.exists(pdf_file_path):
            new_pdf_file_path = os.path.join(os.path.dirname(pdf_file_path), f"{DEFAULT_RESULT_FILE_NAME}.{PDF}")
            os.rename(pdf_file_path, new_pdf_file_path)

    def compress(self, image_file: str, compression_allowed: bool) -> Union[Tuple[str, int, int], str]:
